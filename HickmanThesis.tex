...
\chapter{Application in Curricular Analytics}
  ...
  \section{Correlating Complexity with Completion Rates}
    ...
    As a start, the correlation between student complention rates and structural complexity is examined in very simple curricula which can be seen in figrure \ref{fig:simple}. These curricula, consisting of two terms with two courses each make up common patterns that can be found in actual curricula and span of range of complexities making them good candiates to begin to understand how structure affects students moving through a curriculum.
    ...
    (1) Every courses had a 50\% passrate, giving each student a one in two chance of passing the course. (2) Each student could take up to three courses a term. (3) A duration of four terms.
    ...
    Again, the results imply a correlation between structure and completion rates. To more formally characterize this correlation, linear regression was performed over the data, the details of which can be seen in table \ref{tab:gen6reg} and is visualized by the red line in figure \ref{fig:gen6results}.
